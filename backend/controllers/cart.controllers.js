...
exports.getCartItemCount = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ message: "Error al obtener el conteo del carrito" });
    }
};
...
exports.addItemToCart = async (req, res) => {
    ...
    try {
        ...
        const existingItem = await CartModel.getItemByProductId(userId, productId);
        
        if (existingItem) {
            // Si existe, incrementar la cantidad
            await CartModel.incrementItemQuantity(existingItem.id, cantidad);
